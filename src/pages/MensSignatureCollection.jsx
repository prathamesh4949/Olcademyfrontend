...
const MensSignatureCollection = () => {
  ...
  const fetchMensSignatureScents = useCallback(async () => {
    try {
      ...
      console.error('Error fetching mens signature scents:', err);
...
const ScentCard = memo(({ scent, addToCart, isInCart, toggleWishlist, isInWishlist, navigate, addNotification, setIsCartOpen }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-[#1a1a1a] via-[#2a2a2a] to-[#3a3a3a] dark:from-gray-800 dark:to-gray-700 p-6 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 text-left relative border border-[#444] dark:border-gray-600 group cursor-pointer backdrop-blur-sm flex flex-col"
    ...
    >
      ...
      <motion.button
        ...
        className="absolute top-4 right-4 text-white hover:text-red-600 dark:text-[#f6d110] dark:hover:text-red-400 z-10 bg-black/90 dark:bg-black/90 backdrop-blur-sm rounded-full p-2 shadow-md transition-all duration-200"
      ...
      <AnimatePresence>
        {isHovered && (
          <motion.button
            ...
            className="absolute top-4 right-16 bg-black/90 dark:bg-black/90 backdrop-blur-sm rounded-full p-2 shadow-md transition-all duration-200 z-10"
            aria-label="Quick view"
          >
            <Eye size={18} className="text-white dark:text-[#f6d110]" />
          </motion.button>
        )}
      </AnimatePresence>

      {/* Collection Badge */}
      <div className="absolute top-4 left-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-md z-10 flex items-center space-x-1">
        <Crown size={12} />
        <span>SIGNATURE</span>
      </div>
      ...
      {/* Image Container */}
      <div className="bg-white/10 dark:bg-black/20 backdrop-blur-sm rounded-xl p-4 mb-4 shadow-inner relative overflow-hidden">
        {imageLoading && (
      ...
      <div className="space-y-3 flex-1">
        <div className="flex items-start justify-between">
          <h3 className="text-xl font-alata text-white dark:text-gray-200 font-bold leading-tight">
        ...
        {scent.description && (
          <p className="text-sm text-gray-400 dark:text-gray-400 leading-relaxed line-clamp-2">
            {scent.description}
          </p>
        )}
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex flex-col">
            <p className="text-xl font-bold text-white dark:text-[#f6d110]">
              ${typeof scent.price === 'number' ? scent.price.toFixed(2) : '0.00'}
            </p>
            {scent.originalPrice && scent.originalPrice > scent.price && (
              <p className="text-sm text-gray-400 dark:text-gray-400 line-through">
                ${scent.originalPrice.toFixed(2)}
              </p>
            )}
          </div>
          <div className="bg-purple-500/10 dark:bg-purple-400/10 px-2 py-1 rounded-full">
            <span className="text-xs text-purple-300 dark:text-purple-400 font-medium">SIGNATURE</span>
          </div>
        </div>
      </div>
                ...
                <div className="flex gap-4">
                  {productInQuickViewCart ? (
                    ...
                    <button
                      ...
                      className="flex-1 bg-gradient-to-r from-purple-600 to-indigo-600 text-white py-3 rounded-xl font-semibold hover:shadow-lg transition-all duration-300 flex items-center justify-center space-x-2"
                    >
                      <ShoppingBag size={20} />
        ...
        <section className="relative overflow-hidden w-full bg-gradient-to-br from-purple-900 via-indigo-900 to-blue-900">
          <div className="relative w-full h-[500px] md:h-[600px] lg:h-[700px] flex items-center justify-center">
            <img
              ...
              alt="Men's Signature Collection"
              ...
              onError={(e) => {
                ...
                e.target.parentElement.innerHTML = '<div class="flex items-center justify-center w-full h-full bg-gradient-to-br from-purple-900 to-indigo-900"><div class="text-center"><div class="mx-auto mb-4 text-purple-400"><svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="12" cy="12" r="5"/></svg></div><h2 class="text-4xl font-bold text-white">Men\'s Signature Collection</h2></div></div>';
              }}
            />
            ...
            {/* Men's Signature Collection Text Overlay - Center */}
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center px-6">
                <motion.div
                  ...
                  className="mb-6"
                >
                  <Crown className="w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 mx-auto text-purple-400 drop-shadow-[0_0_15px_rgba(168,85,247,0.8)]" />
                </motion.div>
                ...
                >
                  Discover Sophisticated Scents That Leave a Lasting Impression
                </motion.p>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto">
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <button
                  onClick={fetchMensSignatureScents}
              ...
              <>
                <div className="mb-6 text-center">
                  <p className="text-gray-400">
                    Showing {scents.length} of {totalPages * itemsPerPage} men's signature scents
                  </p>
                </div>
